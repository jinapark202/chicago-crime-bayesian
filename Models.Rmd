...
crime_data_0 <- data.frame(step=1:50000, crime_sim_0[[1]])
```
...
ggplot(Crime_count,aes(x=Year, y=total.stop)) + geom_point() + geom_line() + labs(title="Crime trend over time")
...
```
General trend shows a decrease in crime incidents, however, the variation might be different for different locations.
...
Model_Data_1 <- Model_Data %>%
  ...
  right_join(Crime_rate_log, by="zip") 

head(Model_Data_1)

ggplot(Model_Data_1, aes(x=per_capita_income, y=crime_rate_log, colour=percent_nonwhite)) + geom_point() + labs(x="Per Capita Income", y="Log of Crime Rate", title="Relationship between Per Capita Income and Log of Crime Rate")
```
...
#combining the model (nba_model) and data (y)
#set the random number seed
crime_jags_1 <- jags.model(textConnection(crime_model_1), data=list(y=Crime_count, n=Population$total_population, time=time),
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))


 #simulate a sample from the posterior 
#note that we specify both mu and tau variables
crime_sim_1 <- coda.samples(crime_jags_1, variable.names=c("p", "delta"), n.iter=40000)


#store the samples in a data frame:    
crime_data_1 <- data.frame(step=1:40000, crime_sim_1[[1]])
```
...
```{r}
#running_mean_plot(crime_data_1$alpha)
running_mean_plot(crime_data_1$p.1.1.)
running_mean_plot(crime_data_1$delta.3.)
```
...
income <- Model_Data %>%
  select(per_capita_income, Year,zip)
head(income)
...
crime_model_3 <- "model{
    ...
    for(i in 1:58) {
      for(j in 1:6){
        y[i,j] ~ dbin(p[i,j], n[i])
        logit(p[i,j]) = delta*time[j] + sigma*income[i]
      }
    }
    
    #Priors
    sigma ~ dnorm(0, 1/1000^2)
    delta ~ dnorm(0, 1/1000^2)
...
#combining the model (nba_model) and data (y)
#set the random number seed
crime_jags_3 <- jags.model(textConnection(crime_model_3), data=list(y=Crime_count, 
                                                                    n=Population$total_population, 
                                                                    time=time, 
                                                                    income=income$per_capita_income),
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))


#simulate a sample from the posterior 
#note that we specify both mu and tau variables
crime_sim_3 <- coda.samples(crime_jags_3, variable.names=c("p", "delta", "sigma"), n.iter=30000)


#store the samples in a data frame:    
crime_data_3 <- data.frame(step=1:30000, crime_sim_3[[1]])
...
```{r}
running_mean_plot(crime_data_3$p.1.6.)
running_mean_plot(crime_data_3$p.1.1.)
running_mean_plot(crime_data_3$sigma)
running_mean_plot(crime_data_3$delta)
```

```{r}
ggplot(crime_data_3, aes(x=sigma)) + geom_histogram(aes(y=..density..), colour="white")
ggplot(crime_data_3, aes(x=delta)) + geom_histogram(aes(y=..density..), colour="white")
```
...
Crime_count <- Model_Data %>%
  mutate(percent_nonwhite = 100 - percent_white) %>%
  select(zip, Year, total, percent_nonwhite, per_capita_income, total_population) 
Crime_count[is.na(Crime_count$total)] <- 0
head(Crime_count)
...
#combining the model (nba_model) and data (y)
#set the random number seed
#quantile(Crime_count$percent_nonwhite, c(0.33, 0.66)) >> 41.00, 81.66
#66% or more nonwhite
crime_jags_3.1 <- jags.model(textConnection(crime_model_3), data=list(y=Crime_count %>% filter(percent_nonwhite > 81.66) %>% select(-percent_nonwhite), 
                                                                      n=Population$total_population, 
                                                                      time=time, 
                                                                      income=Model_Data$per_capita_income),
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))
#33%-66% nonwhite
crime_jags_3.2 <- jags.model(textConnection(crime_model_3), data=list(y=Crime_count %>% filter(percent_nonwhite > 41 & percent_nonwhite <= 81.66) %>% select(-percent_nonwhite), 
                                                                      n=Population$total_population, 
                                                                      time=time, 
                                                                      income=Model_Data$per_capita_income),
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))
#less than 33% nonwhite
crime_jags_3.1 <- jags.model(textConnection(crime_model_4), data=list(y=Crime_count, n=Population.vector, time=time, income=Model_Data$per_capita_income, percent_nonwhite=percent_nonwhite),
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))


#simulate a sample from the posterior 
#note that we specify both mu and tau variables
crime_sim_3.1 <- coda.samples(crime_jags_4, variable.names=c("p", "delta", "sigma", "rho"), n.iter=40000)
crime_sim_3.1 <- coda.samples(crime_jags_4, variable.names=c("p", "delta", "sigma", "rho"), n.iter=40000)
...
```{r}
running_mean_plot(crime_data_4$p.1.1)
running_mean_plot(crime_data_4$sigma)
running_mean_plot(crime_data_4$delta)
running_mean_plot(crime_data_4$rho)
```
