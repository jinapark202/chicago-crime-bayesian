...
- Produced several models for each of our research questions.
...
- We worked on formulating the models together by reading past research and altering it with our questions.
...
crime_model_4 <- "model{
    ...
    for(i in 1:58) {
      for(j in 1:8){
        y[i,j] ~ dbin(p[i,j], n[i,j])
        logit(p[i,j]) = alpha + u[i] + s[i]+ delta[i]*time[j] + sigma[i]*income[i] + rho[i]*percent_nonwhite[i]
      }
    }
...
#simulate a sample from the posterior 
#note that we specify both mu and tau variables
crime_sim_4 <- coda.samples(crime_jags_4, variable.names=c("p", "u", "s", "delta", "alpha", "sigma", "rho"), n.iter=40000)
...
police_model_1 <- "model{
    ...
    for(i in 1:4) {
      for(j in 1:58){
        y[i,j] ~ dpois(lam[i, j])
        lam[i, j] <- (13/12)*n[i,j]*exp(mu + alpha[i] + beta[j])
      }
    }
    ...
    # for (i in 1:4){
    #   for (j in 1:62){
    #     epsilon[i,j] ~ dnorm(0, 1/pow(sigma_epsilon, 2))
    #   }
    # }

    #Hyperpriors
    sigma_beta ~ dunif(0, 1000)
    sigma_epsilon ~ dunif(0, 1000)
...
police_model_1 <- "model{
  ...
  #Hyperpriors
  sigma_beta ~ dgamma(0.5, 0.0005)
  sigma_epsilon ~ dgamma(0.5, 0.0005)
...
#simulate a sample from the posterior 
#note that we specify both mu and tau variables
police_sim_1 <- coda.samples(police_jags_1, variable.names=c("mu", "alpha", "beta", "c"), n.iter=10000)
