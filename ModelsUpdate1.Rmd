...
ggplot(Model_Data_1, aes(x=log(per_capita_income), y=log_rate, colour=percent_nonwhite)) + geom_point() + labs(x="Log of Per Capita Income", y="Log of Crime Rate", title="Relationship between Per Capita Income and Crime Rate") +
...
crime_model_1 <- "model{
    ...
    for(i in 1:ns) {
      y[i] ~ dbin(p[i], n[i])
      logit(p[i]) = alpha[zipid[i]] + delta[zipid[i]]*time[i]
    }
    ...
    #Hyperpriors
    tau_delta ~ dgamma(0.5, 0.0005)
    tau_alpha ~ dgamma(0.5, 0.0005)
...
crime_sim_1 <- coda.samples(crime_jags_1, variable.names=c("delta", "alpha", "tau_delta", "tau_alpha"), n.iter=30000)
...
running_mean_plot(crime_data_1$tau_delta)
running_mean_plot(crime_data_1$tau_alpha)
running_mean_plot(crime_data_1$alpha.1.)
```
...
```{r message=FALSE, warning=FALSE}
ggplot(crime_data_1, aes(x=delta.1.)) + geom_histogram(aes(y=..density..), colour="white") + labs(title="Posterior Distribution of the Crime Differential Growth in 1st zipcode in 2010", x="Crime Differential Growth", y="Density")
```
...
png("crime differential growth by zip.png")
model_2_map <- ggplot() 
...
model_2_map <- model_2_map + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_rect(fill = 'white', colour = 'white'),
        axis.line = element_line(colour = "white"),
        axis.ticks=element_blank(), axis.text.x=element_blank(),
        axis.text.y=element_blank())
...
data.1 = list(y=Model_Data.1$total, n=Model_Data.1$total_population, time=Model_Data.1$time, income=Model_Data.1$income, zipid=Model_Data.1$id, ns=length(Model_Data.1$total),nzip=unique(Model_Data.1$id))
...
crime_model_3 <- "model{
    ...
    for(i in 1:ns) {
      y[i] ~ dbin(p[i], n[i])
      logit(p[i]) = beta[zipid[i]] + delta[zipid[i]]*time[i] + sigma*income[i]
    }
...
crime_sim_3.1 <- coda.samples(crime_jags_3.1, variable.names=c("delta","sigma", "prec.delta", "beta","prec.beta"), n.iter=30000, n.thin=5)
crime_sim_3.2 <- coda.samples(crime_jags_3.2, variable.names=c("delta","sigma", "prec.delta", "beta","prec.beta"), n.iter=30000)
crime_sim_3.3 <- coda.samples(crime_jags_3.3, variable.names=c("delta","sigma", "prec.delta", "beta","prec.beta"), n.iter=30000)
...
ggplot(crime_data_3.1, aes(x=1:30000, y=delta.4.)) + geom_line()
running_mean_plot(crime_data_3.1$sigma)
running_mean_plot(crime_data_3.1$prec.delta)
running_mean_plot(crime_data_3.1$beta.4.)
```
```{r}
#check for convergece
#from: https://rpubs.com/corey_sparks/32494
init.rng1<-list(".RNG.seed" = 1234, ".RNG.name" = "base::Mersenne-Twister")
init.rng2<-list(".RNG.seed" = 5678, ".RNG.name" = "base::Mersenne-Twister")
mod<-jags.model(file=textConnection(crime_model_3), data=data.1 ,inits =list(init.rng1, init.rng2) , n.chains=2)
#update(mod, 10000)
samps<-coda.samples(mod, variable.names=c("delta","sigma", "prec.delta", "beta","prec.beta"), n.iter=5000, n.thin=5)
gelman.diag(samps)
```
