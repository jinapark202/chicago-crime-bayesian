...
ggplot(Model_Data_1, aes(x=log(per_capita_income), y=crime_count_log, colour=percent_nonwhite)) + geom_point() + labs(x="Log of Per Capita Income", y="Log of Crime Rate", title="Relationship between Per Capita Income and Crime Rate", color="Nonwhite Percentage") 
```

**Plot of $log(y_{ij}/n_{i})$ vs nonwhite percentage**
```{r}
ggplot(Model_Data_1, aes(x=log(percent_nonwhite), y=crime_count_log, colour=per_capita_income)) + geom_point() + labs(x="Log of Nonwhite Percentage", y="Log of Crime Rate", title="Relationship between Nonwhite Percentage and Log of Crime Rate")
```
...
crime_model_1 <- "model{
    ...
    for(i in 1:58) {
      for(j in 1:8){
        ...
        logit(p[i,j]) = beta[i] + delta[i]*time[j]
      }
    }
...
crime_jags_1 <- jags.model(textConnection(crime_model_1), data=list(y=Crime_count, n=Population.vector, time=time),
 ...
 #simulate a sample from the posterior 
#note that we specify both mu and tau variables
crime_sim_1 <- coda.samples(crime_jags_1, variable.names=c("p", "delta", "beta", "tau0", "tau1"), n.iter=30000)
...
running_mean_plot(crime_data_1$tau0)
running_mean_plot(crime_data_1$tau1)
```
...
$$y_{ij} \sim Bin(p_{ij}, n_{i})$$
$$logit(p_{ij}) = \beta_{i} + \delta_{i} * time_{j} + \sigma * income_{i}$$

#### Priors
$$\delta_{i} \sim N(0, \tau_{1})$$
$$\sigma_{i} \sim N(0, \tau_{2})$$
...
crime_model_3 <- "model{
    ...
    for(i in 1:58) {
      for(j in 1:8){
        y[i,j] ~ dbin(p[i,j], n[i])
        logit(p[i,j]) = beta[i] + delta[i] * time[j] + sigma * income[i]
      }
    }
    
    #Priors
    sigma ~ dnorm(-0.005, tau2)
    for(i in 1:58){
      beta[i] ~ dnorm(0, tau0)
      delta[i] ~ dnorm(-0.005, tau1)
    }
    ...
    #Hyperpriors
    tau0 ~ dgamma(0.05, 0.0005)
    tau1 ~ dgamma(0.05, 0.0005)
    tau2 ~ dgamma(0.05, 0.0005)
...
#simulate a sample from the posterior 
#note that we specify both mu and tau variables
crime_sim_3 <- coda.samples(crime_jags_3, variable.names=c("p", "delta", "sigma", "beta", "tau0", "tau2", "tau1"), n.iter=60000)
...
police_model_0 <- "model{
  ...
  #Priors
  mu ~ dnorm(0.005, 0.0005)

  for (i in 1:4){
    alpha[i] ~ dnorm(-0.005, 0.0005)
  }
}"
...
#simulate a sample from the posterior 
#note that we specify both mu and tau variables
police_sim_0 <- coda.samples(police_jags_0, variable.names=c("mu", "alpha"), n.iter=60000)
...
running_mean_plot(police_data_0$alpha.1.)
```
...
police_model_1 <- "model{
  ...
  for (j in 1:58){
    beta[j] ~ dnorm(-0.005, tau0)
  }

  for (i in 1:4){
    for (j in 1:58){
      epsilon[i,j] ~ dnorm(0, tau1)
    }
  }

  tau0 ~ dgamma(0.5, 0.0055)
  tau1 ~ dgamma(0.5, 0.0055)
...
#simulate a sample from the posterior 
#note that we specify both mu and tau variables
police_sim_1 <- coda.samples(police_jags_1, variable.names=c("mu", "alpha", "beta", "sd_beta", "sd_epsilon", "epsilon"), n.iter=60000)
