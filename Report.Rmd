...
The figure above shows the trend in log of crime rate over time. Thought there is a lot of noise, we think it is reasonable to model log of crime rate as a linear function of time, and that each zipcode seeme to have a different slope and intercept.
...
In this model, $y_{ij}$ is the observed crime count in zipcode $i$ and year $j$, where $p_{ij}$ is the probability of a person being involved in a crime, or the inherent crime rate, and $n_{ij}$ represents the total population of zipcode {i} and year {j}. For our analysis, we assume that population and other demographics information of each zipcode remain unchanged from the 2010 Census. The number of crime count is modeled with a Binomial distribution because a person is either involved in a crime incident or not involved in a crime incident. 

$log(\frac{p_{ij}}{1-p_{ij}})$ represents the log odds of the inherent crime rate in zipcode $i$ and year $j$. The log odds is a linear function of time with $\beta_{i}$, which is the intercept for each zipcode $i$ in Chicago, $\delta_{i}$, the crime differential growth over time in zipcode $i$, and time. Thus, we assume here that each zipcode has a different change in crime rate over time.

The prior distributions of $\beta_i$ are normally distributed with a mean of $\beta_0$, the grand intercept, and a precision of $\tau_0$. Similarly, prior distributions of $\delta_i$ are normally distributed with a mean $\delta_0$, the grand time trend, and precision $\tau_1$. Precisions $\tau_{0,1}$ are modeled with a Gamma distribution. The mean of this Gamma distribution is 0.00025 equivalent to a mean standard deviation of 63 for $\beta_i, \delta_i$'s normal distributions. This signifies that $\tau$ is a vague prior given that the grand mean of $\beta_{i}$ and $\delta_{i}$ are approximately -7.49 and -0.066. The priors for $\beta_0, \delta_0$ are also vague with mean at 0 and standard deviation 1000.

**This is the zipcode specific time trend for log odds crime rate, the mean change is delta0 = -0.066 every new year **
...
Using Model 1, the map above shows the crime differential growth by zipcode. In all areas of Chicago, the overall number of crime decreases over time. The crime hot spots are shown in areas highlighted in bright orange where crime is not decreasing as much as other areas over time.
...
Change color scale for violent and non-violent crime.
