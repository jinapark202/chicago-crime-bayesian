...
**Add lit review about demographics**
...
```{r, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE,fig.width=10, fig.height=7, fig.align="center"}
...
The prior distributions of $\beta_i$ are normally distributed with a mean of $\beta_0$, the grand intercept, and a precision of $\tau_0$. Similarly, prior distributions of $\delta_i$ are normally distributed with a mean $\delta_0$, the grand time trend, and precision $\tau_1$. Precisions $\tau_{0,1}$ are modeled with a Gamma distribution. The mean of this Gamma distribution is 0.00025, which is equivalent to a mean standard deviation of 63 for $\beta_i, \delta_i$'s normal distributions. This signifies that $\tau$ is a vague prior given that the grand mean of $\beta_{i}$ and $\delta_{i}$ are approximately -7.49 and -0.066. This means that the average change in the crime rate over time is roughly -0.66. The standard deviation of the change in the crime rate over time is 0.023. The priors for $\beta_0$ and  $\delta_0$ are also vague with a mean of 0 and standard deviation of 1000.

Based on the 95% confidence intervals of our parameters, we conclude that all of the intercepts and the vast majority of our parameters in Model 1 are significant at the 5% level. 
...
```{r, message=FALSE, warning=FALSE, cache=TRUE, echo=FALSE, fig.width=7, fig.height=7, fig.align="center"}
#Make a map
model_1_map <- ggplot() 
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, comment=NA, fig.width=10, fig.height=7, fig.align="center"}
...
Based on the 95% confidence intervals of our parameters, we conclude that all of the intercepts and the vast majority of our parameters in Model 2 are significant at the 5% level. Areas with 60-80% and less than 20% white populations are considered significant at the 5% level. This means that there is a significant relationship between crime rate and income in these area. However, all other race groups are not considered significant at the 5% level. 
...
```{r, cache=TRUE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=7, fig.height=7, fig.align="center"}
model_2_map <- ggplot() 
...
For violent crime, in region with 60-80% white, posterior mean for $\sigma$, the relationship between violent crime rate and income, is 0.365 (se=6.1e-04). While in region with lower than 20% white, this relationship is -1.192 (se=1.22e-03). Only these two regions have statistically significant parameters.

For non-violent crime, in region with 60-80% white, posterior mean for this relationship is 0.738 (se=7.03e-03) and in region with 40-60% white is 0.735 (se=2.22e-03). In region with lower than 20% white, the posterior mean is is -0.687 (se=1.539e-03). Only the reigions discussed here have statistically significant parameters at 95% confidence interval.

Although the number of violent crimes in areas with large white populations are growing, they grow at a slower rate compared to non-violent crimes as these areas' per capita income increases. Areas whose residents are predominantly people of color, as income increases, crime rate, particularly violent crime rate decreases. 
...
model_2_map_v <- model_2_map_v + labs(x="", y="", title=paste("Change in Violent Crime Rate as Income Increases"), fill="Trend over Income", caption="Model 2")
...
model_2_map_nv <- model_2_map_nv + labs(x="", y="", title=paste("Change in Non-Violent Crime Rate as Income Increases"), fill="Trend over Income", caption="Model 2")
...
```{r, echo=FALSE, cache=TRUE, message=FALSE, warning=FALSE, fig.width=7, fig.height=7, fig.align="center"}
model_3_map <- ggplot() 
...
In general, the crime rate in Chicago has decreased from 2010-2015. However, when accounting for violent and non-violent crimes, we see increases in violent crimes in all areas and non-violent crimes in areas with large white populations.

### Next Steps

**Discuss future work that could be done on your project. What are the limitations of your current data/models/algorithms?**

In the future, we are interested in examining hot spots for specific types of crime in Chicago. For instance, how are hot spots different for homicides compared to petty theft? Are certain ethnic or income groups more likely to commit certain crimes than others?

In some plots, it would be easier to see the difference between rate of change of violent and nonviolent crimes if the maps have the same color scale. We would also like to indicate where particular groups are on the map.

We used zipcodes in our analysis due to its familiarity to general audiences. We could potentially repeat the same analysis using tract data since tracts tend to have more consistent boundaries, similar populations, and is the official unit by which the U.S. Census data are collected. 

Our data is geographical. Although we have random effects for each zipcode in the analysis, we can also expect to have autocorrelated random effects between zipcodes that are adjacent. Adding a random effect term to account for this could help smooth out dramatic differences in change of crime rate in adjacent locations. 
