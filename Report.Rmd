...
data.2 = list(y=Model2_Data$total, 
...
temp.V <- Model_Data %>% filter(!duplicated(zipid))

Model2_Data_Violent <- Model_Data_Violent %>% filter(Year==2010)
temp.NV <- Model_Data %>% filter(!duplicated(zipid))

data.V.2 = list(y=Model2_Data_Violent$total, 
            n=Model2_Data_Violent$total_population, 
            zipid=Model2_Data_Violent$zipid, 
            racegr=Model2_Data_Violent$racegr, 
            income=Model2_Data_Violent$income, 
            ns=nrow(Model2_Data_Violent), 
            time=Model2_Data_Violent$time, 
            nzip=length(unique(Model2_Data_Violent$zip)),
            raceid = temp.V$racegr,
            incomezip = temp.V$income)

data.NV.2 = list(y=Model2_Data_NViolent$total, 
            n=Model2_Data_NViolent$total_population, 
            zipid=Model2_Data_NViolent$zipid, 
            racegr=Model2_Data_NViolent$racegr, 
            income=Model2_Data_NViolent$income, 
            ns=nrow(Model2_Data_NViolent), 
            time=Model2_Data_NViolent$time, 
            nzip=length(unique(Model2_Data_NViolent$zip)),
...
crimeV_sim_2 <- coda.samples(crimeV_jags_2, variable.names=c("beta", "beta0", "delta", "delta0","sigma","sigma0", "tau_0","tau_1","tau_2"), n.iter=50000)
crimeNV_sim_2_ <- coda.samples(crimeNV_jags_2, variable.names=c("beta", "beta0", "delta", "delta0","sigma","sigma0", "tau_0","tau_1","tau_2"), n.iter=50000)
...
crime_model_3 <- "model{
    ...
    delta0 ~ dnorm(0, 1/1000^2)
    for(i in 1:nzip){
      beta[i] ~ dnorm(beta0 + sigma[2]*whitezip[i] + sigma[3]*interactionzip[i] + sigma[1]*incomezip[i], tau_0)
      delta[i] ~ dnorm(delta0, tau_1)
    }
    for(i in 1:3){
...
data.V = list(y=Model_Data_Violent$total, n=Model_Data_Violent$total_population, time=Model_Data_Violent$time, zipid=Model_Data_Violent$id, ns=length(Model_Data_Violent$total), nzip=length(unique(Model_Data_Violent$id)))
