...
```{r}
ggplot(Crime, aes(x=Primary.Type, fill=as.factor(Arrest))) + geom_bar() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
```{r}
ggplot(Crime, aes(x=Location.Description, fill=as.factor(Arrest))) +   geom_bar() +  
  theme(axis.text.x = element_text( angle = 90, hjust = 1)) + 
...
```{r}
ggplot(Crime, aes(x=Location.Description, fill=as.factor(Domestic))) +   geom_bar() +  
  theme(axis.text.x = element_text( angle = 90, hjust = 1)) + 
...
for (i in 1:length(crime_types)){
  ...
  if (nrow(locations) > 0){
    ...
    chicago_ts <- ggplot() 
    chicago_ts <- chicago_ts +  geom_polygon(data = total_map, aes(x=long, y=lat, group=group, fill=total), color = "black", size=0.2) 
    chicago_ts <- chicago_ts + coord_map() 
    chicago_ts <- chicago_ts + labs(title=paste("Crimes of type", toString(current_type), sep=" "), fill="total")
    chicago_ts <- chicago_ts +  theme_nothing(legend=TRUE) 
    print(chicago_ts)
  }
}
```
