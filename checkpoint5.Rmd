...
crime_model_1 <- "model{
    ...
    for(i in 1:58) {
      for(j in 1:6){
        ...
        logit(p[i,j]) = delta[i]*time[j] + u[i]
      }
    }
...
crime_jags_1 <- jags.model(textConnection(crime_model_1), data=list(y=Crime_count, n=Population$total_population, time=time),
 ...
 #simulate a sample from the posterior 
#note that we specify both mu and tau variables
crime_sim_1 <- coda.samples(crime_jags_1, variable.names=c("p", "delta", "u"), n.iter=70000)


#store the samples in a data frame:    
crime_data_1 <- data.frame(step=1:70000, crime_sim_1[[1]])
```
...
```{r}
running_mean_plot(crime_data_1$u.3.)
