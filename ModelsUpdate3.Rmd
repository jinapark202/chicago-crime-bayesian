...
### Mean of parameters from Model 4
```{r}
head(crime_data_4)
crime_data_4 <- crime_data_4[20000:nrow(crime_data_4), ]
crime_data_4_sum <- as.data.frame(colMeans(crime_data_4))
crime_data_4_sum
```
...
```{r}
model_4_predictor <- function(param, income, percent_nonwhite, year){
   return (param[1,1] + param[2,1] * year + param[3,1] * income + param[4,1] * percent_nonwhite + param[5,1] * percent_nonwhite * income)
 }
```

```{r}
predicted_crime_rate <- model_4_predictor(crime_data_4_sum, test_data$income, test_data$percent_white, test_data$time)
