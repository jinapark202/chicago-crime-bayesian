...
data = list(y=Model3_Data_Violent$total, n=Model3_Data_Violent$total_population, zipid=Model3_Data_Violent$id, racegr=Model3_Data_Violent$racegr, income=Model3_Data_Violent$income, ns=nrow(Model3_Data_Violent), nzip=length(unique(Model3_Data_Violent$zip)))
data.NV = list(y=Model3_Data_NViolent$total, n=Model3_Data_NViolent$total_population, zipid=Model3_Data_NViolent$id, racegr=Model3_Data_NViolent$racegr, income=Model3_Data_NViolent$income, ns=nrow(Model3_Data_NViolent), nzip=length(unique(Model3_Data_NViolent$zip)))
```
...
#combining the model (nba_model) and data (y)
#set the random number seed
crimeV_jags_3 <- jags.model(textConnection(crime_model_3), data=data,
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))
crimeNV_jags_3 <- jags.model(textConnection(crime_model_3), data=data.NV,
                    inits=list(.RNG.name="base::Wichmann-Hill", .RNG.seed=1989))


#simulate a sample from the posterior 
#note that we specify both mu and tau variables
crimeV_sim_3 <- coda.samples(crime_jags_3, variable.names=c("sigma", "tau_0", "beta","tau_1"), n.iter=30000, n.thin=5)
crimeNV_sim_3 <- coda.samples(crimeNV_jags_3, variable.names=c("sigma", "tau_0", "beta","tau_1"), n.iter=30000, n.thin=5)


#store the samples in a data frame:    
crimeV_data_3 <- data.frame(crime_sim_3[[1]])
...
model_3_map <- model_3_map + coord_map() 
model_3_map <- model_3_map + labs(x="", y="", title=paste("Violent Crime Differential Growth Based on Income and Demographics"), fill="total")
model_3_map <- model_3_map + cleanup
print(model_3_map)
```
...
model_3_map <- model_3_map + coord_map() 
model_3_map <- model_3_map + labs(x="", y="", title=paste("Non-violent Crime Differential Growth Based on Income and Demographics"), fill="total")
model_3_map <- model_3_map + cleanup
print(model_3_map)
